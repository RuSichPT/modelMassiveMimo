function simulateMutCorr(obj, rangeSNR, maxNumZeroBER, minNumErrs, maxNumSimulation, alpha, betta)
    ...
    for indSNR = 1:length(rangeSNR)
        if (numZeroBER < maxNumZeroBER) 
            ...
            while ( condition && (indSim < maxNumSimulation) )        
                [numErrors, numBits] = obj.simulateOneSNRmutCorr(rangeSNR(indSNR), alpha, betta);
